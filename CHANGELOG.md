...
- Automated changelog generation workflow using AI SDK and Anthropic API
- Node.js configuration with `.node-version` file and `package.json` dependencies
- Environment configuration with `.env.example` template
- `make changelog` target for manual changelog generation

### Changed

- CI workflow now tests Docker code path with `ASYLUM_DOCKER=1`
- Updated setup-node action to v6 in changelog workflow
- Changelog workflow now skips pre-releases

### Fixed

- Install script referencing wrong path for statusline config
